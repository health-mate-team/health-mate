...
| 있음 + 명세 변경 감지 | API_SPEC diff 표시 + 카탈로그 갱신 동의 (--auto면 자동 갱신) |
...
| 상태 | 동작 |
|---|---|
| 없음 또는 `dynamic.ui_scenario` 비어 있음 | **§5-1a 자동 생성 파이프라인** 실행 |
| 있음 + 화면 코드 변경 감지 | ui_scenario 갱신 후보 제시 후 동의 (--auto면 자동 갱신) |
| 있음 + 변경 없음 | 그대로 사용 |

### 5-1a. 화면 ui_scenario 자동 생성 파이프라인

> screen 타겟이고 `dynamic.ui_scenario`가 없을 때 실행 (spec §6.3a 기준).

**1. ROUTE 확인**: `_routing-map.yaml` → `screens.{name}.route` 읽기

**2. Dart 파일 분석** (presentation layer):
```
분석 대상:
  initState / Future.microtask → ref.read(provider.notifier).load()
    → Repository 메서드 → GET API endpoint   [초기 로드 API]

  Button/FilledButton/OutlinedButton.onPressed → handler()
    → ref.read(provider.notifier).action() → POST/PATCH API  [액션 API]

  AppBar.title → Text('...') 리터럴   [heading 텍스트]
  Text('${data.field}') 보간 패턴     [데이터 렌더링]
  if (data.isEmpty) const Text('...')  [빈 상태 텍스트]
```

**3. API 명세 매핑**: spec_section 읽어 각 endpoint의 성공 status, 응답 필드 파악

**4. 케이스 도출 규칙**:

| 케이스 | 트리거 | 생성 steps |
|---|---|---|
| Golden Path (GET 로드) | initState API 호출 | navigate → wait_for(GET) → snapshot → assert_network(GET,200) → assert_console(0) |
| Empty state | 빈 상태 텍스트 존재 | Golden Path + wait_for(text: 빈상태텍스트) |
| Primary action (POST) | 주요 버튼 onPressed | Golden Path 완료 가정 → click(버튼라벨) → wait_for(POST) → assert_network(POST,200) → assert_console(0) |
| Secondary action | 보조 버튼 (건너뛰기 등) | 별도 `ui_scenario_skip` 키로 분리 |

**5. YAML 생성**: `docs/verify-cases/screen_{name}.yaml` 저장 + `_routing-map.yaml`에 `ui_scenario_catalog` 등록

**생성 원칙:**
- `assert_console(error, 0)` 은 항상 마지막 step
- 버튼→API 매핑 신뢰도 LOW → `# TODO: 조건부 트리거 확인 필요` 주석 삽입
- wait_for timeout: GET 10s, POST 8s
- 직접 라우트 진입 시 `context.pop()` 대신 `context.canPop() ? pop() : go('/home')` 패턴 권고 주석 추가
...
- **함정/선례**: 자동 캡처 슬롯 폐기 (2026-05-08, F-02 closure). 가차스/놀리지는 cc auto memory + 사용자 명시 메모리로 운영. 자세한 결정 근거: `~/.claude/projects/-mnt-d-workspace/memory/feedback_auto_capture_closure.md`
